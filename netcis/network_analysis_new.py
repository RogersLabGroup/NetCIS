...
def compare_pcis(target_overlaps, target_subgraphs, reference_subgraphs, target, reference, chrom):
    ...
    for tar_ind, ref_inds in target_overlaps.items():
        ...
        tmp_tar = pd.DataFrame([ {"target_count": tar_G.nodes[node]['CPM']} for node in tar_G.nodes ], index=tar_pos)        
        if len(ref_inds) == 0:
            tmp = tmp_tar
            tmp["reference_count"] = 0.0
            ...
            for ref_ind in ref_inds:
                ...
                tmp_control = pd.DataFrame([ {"reference_count": ref_G.nodes[node]['CPM']} for node in ref_G.nodes ], index=ref_pos)
        ...
        tmp["p_target_binom_pval"] = tmp.apply(lambda x: binomtest(int(x["target_count"])+1, (int(x["target_count"])+1) + (int(x["reference_count"])+1)).pvalue, axis=1)
        ...
        binom = binomtest(int(tmp["target_count"].sum()), int(tmp["target_count"].sum()) + int(tmp["reference_count"].sum()), 0.5).pvalue
...
def main(args):
    ...
    for treatment in graph_stats_df["type"].unique():
        ...
        simple_summary[treatment] = {"pCIS_count": len(treatment_df),
                                    ...
                                    "norm_reads_count": treatment_df["norm_num_inserts"].sum(),
                                    }
